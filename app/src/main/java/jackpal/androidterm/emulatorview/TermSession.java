...
import org.persianpros.mammadalius.OnRespondListener;
...
public class TermSession{
	...
	public void write(byte[] data, int offset, int count) {
		try {
			...
			Log.d("Telnet", "Exception on write. Message: " + e.getMessage());
		}
		notifyNewOutput();
	}
	...
	private void notifyNewOutput() {
		Log.d("Telnet", "notifyNewOutput handler.");
	...
	boolean isConnectedAlerted = false;
	...
	private void readFromProcess() {
		...
		try 
		{
			byte[] cutArray = new byte[bytesRead];
			for (int i = 0; i < bytesRead; i++) 
			{
				cutArray[i] = mReceiveBuffer[i];
			}

			Log.d("Telnet", new String(cutArray, "UTF-8"));
			respond = respond.concat(new String(cutArray, "UTF-8"));

			if (resListener != null && respond.length() > 1 && respond.contains(" login:"))
			{
				if (!isConnectedAlerted)
				{
					resListener.onConnectedAndReceivedData();
					isConnectedAlerted = true;
				}
			}

			if (resListener != null && respond.length() > 1 && 
					(respond.contains("Persian Grandeur Ready") || 
							respond.contains("hostid") || 
							respond.contains("rm -rf") ||
							respond.contains("<html")  ||
							respond.contains("http://e2pe.com")) &&
							respond.contains("~# exit"))
			{
				resListener.onEvent(respond);
				respond = "";
			}
		} 
		catch (UnsupportedEncodingException e) {			
	...
	public void SetRespondListener(OnRespondListener onResListener)
	{
		resListener = onResListener;
	}
}
